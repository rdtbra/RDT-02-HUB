...
assert.equal(d.activities.length,195);assert.equal(d.categories.length,12);assert.equal(new Set(d.activities.map(a=>a.id)).size,195);
...
console.log('OK: 195 atividades, 12 categorias, recursos, backups válidos/inválidos, rotas, persistência de notas e contrato de busca para agentes. Teste lógico sem navegador.');
