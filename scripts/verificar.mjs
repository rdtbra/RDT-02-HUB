...
assert.equal(d.activities.length,193);assert.equal(d.categories.length,12);assert.equal(new Set(d.activities.map(a=>a.id)).size,193);
...
console.log('OK: 193 atividades, 12 categorias, recursos, backups válidos/inválidos, rotas, persistência de notas e contrato de busca para agentes. Teste lógico sem navegador.');
