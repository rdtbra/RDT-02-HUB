...
assert.equal(d.activities.length,196);assert.equal(d.categories.length,12);assert.equal(new Set(d.activities.map(a=>a.id)).size,196);assert.equal(d.activities.find(a=>a.category==='projetos').id,'rdt-02-hub');
...
console.log('OK: 196 atividades, 12 categorias, recursos, backups válidos/inválidos, rotas, persistência de notas e contrato de busca para agentes. Teste lógico sem navegador.');
